...
public class FullAssociative_L2 {
    static class FullAssociativeCache_L2{
        int set;
        int tag;
        int word;
        int data;
        ...
        public FullAssociativeCache_L2(int k, int t,int w, int data) {
            this.set = k;
            this.tag= t;
            this.word=w;
            this.data = data;
    ...
    private FullAssociativeCache_L2[][] cacheEntries;

    public void initialise(){
        for(int i=0;i<this.numSets;i++) {
            FullAssociativeCache_L2 cacheEntry = new FullAssociativeCache_L2(i, -1, -1, 0);
    ...
    public FullAssociative_L2(int sizeB,int numSets, int sizeOfSet, ReplacementPolicy replacementPolicy) {
        this.sizeOfBlock=sizeB;
        this.numSets = numSets;
        this.sizeOfSet = sizeOfSet;
        this.cacheEntries = new FullAssociativeCache_L2[numSets][sizeOfSet];
        this.replacementPolicy = replacementPolicy;
    }

    public int get(int tag,int setNum,int word) {
        int set=setNum%numSets;
        for(FullAssociativeCache_L2 entry : cacheEntries[set]) {
            if(entry != null) {
    ...
    public void put(int setNum, int tag, int word,int data) {
        ...
        FullAssociativeCache_L2 cacheEntry = new FullAssociativeCache_L2(set, tag, word, data);
        ...
        for (int i = 0; i < this.sizeOfSet; i++) {
            if (cacheEntries[set][i].getValue() == 0) { // To search for an empty space to fill in the data
                ...
                for(int j=i+1;j<i+sizeOfBlock;j++){
                    cacheEntries[set][j]= new FullAssociativeCache_L2(set, tag, -1, 1);
                }
                return;
            }
        }
    ...
    public void LRU(int set){
        ...
        if (sizeCounter==this.sizeOfSet && this.replacementPolicy == ReplacementPolicy.LRU) { // If all space in assigned set is FULL, so Replace using LRU
            FullAssociativeCache_L2 minimumCacheEntry;
            FullAssociativeCache_L2 LatestCacheEntry=cacheEntries[set][0];
    ...
    public void displayCache_L2(int k){
        for(int i=0;i<numSets;i++){
            ...
            for(FullAssociativeCache_L2 entry : cacheEntries[i]) {
